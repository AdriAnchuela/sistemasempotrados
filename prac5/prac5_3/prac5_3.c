...
static inline uint32_t get_ticks_per_second(void) {
	...
	//Retornar los ticks por segundo
...
static inline uint32_t get_ticks_since_boot(void) {
	...
	//Retornar los ticks desde el inicio del sistema
...
rtems_task Init(rtems_task_argument argument) {
	...
	//Fijar el time of day a 22/04/2022 utilizando rtems_clock_set
	time.year = 2022;
	time.month = 04;
	time.day = 22;
	time.hour = 0;
	...
	Task_name[2] = rtems_build_name('T', '2', ' ', ' ');
	Task_name[3] = rtems_build_name('T', '3', ' ', ' ');
	Task_name[4] = rtems_build_name('T', '4', ' ', ' ');
	...
	status = rtems_task_create(Task_name[4], 1, RTEMS_MINIMUM_STACK_SIZE * 2,
	RTEMS_DEFAULT_MODES,
...
rtems_task TAvoidObstacles(rtems_task_argument unused) {
	...
	uint32_t task_index;
	rtems_status_code status;
	rtems_interval current_time, next_time;
	...
	for (;;) {
		...
		rtems_semaphore_obtain(MutexName,RTEMS_NO_WAIT,RTEMS_NO_TIMEOUT);
		puts("T1 Do Avoid Obstacles");
		printf(" - rtems_ticks_since_boot - %i\n", get_ticks_since_boot());
		rtems_semaphore_release(MutexName);
		status = task_delay_until(next_time);
	}
